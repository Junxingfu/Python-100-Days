...
```Python
#列表推导式（List Comprehension）：对于给定的范围，将每个元素提取出来，并将其添加到列表中
#range(1, 10) 生成一个整数序列，for x in range(1, 10) 是一个循环语句
...
```Python
# yield 用于生成器函数，允许函数在产生值的同时保留状态，
# 而 return 用于正常的函数，表示函数的结束并返回一个值
def fib(n):
    a, b = 0, 1
    for _ in range(n):
        a, b = b, a + b
...
def which_day(year, month, date):
    ...
    :return: 第几天
    """
    #days_of_month[is_leap_year(year)] 就会返回相应年份的每个月的天数列表。
    #其实就是if-else的简写版
    days_of_month = [
...
```Python
#yh[row] = [None] * (row + 1)：这行代码使用列表生成式创建一个包含 (row + 1) 个 None 元素的列表，
# 并将其赋值给 yh[row]。这一步是为了确保每一行的列表都有足够的长度，
# 因为杨辉三角的第 row 行有 row + 1 个元素。
def main():
